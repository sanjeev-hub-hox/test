...
import { convertDateDD, getObjectByKeyValNew } from 'src/utils/helper'
...
export default function DynamicFilterComponent(props: FilterProps) {
  ...
  const handleCoulmnFilter = (status: any) => {
    ...
    if (index >= 0) {
      let operatorSet
      filterSectionData.map((value: any) => {
  ...
  const handleMultiSelectChipToggle = (chipValue: any) => {
            ...
            <TabPanel value={value} index={0}>
              <Box sx={{ maxHeight: 'calc(100vh - 150px)', overflow: 'auto' }}>
                <Card variant='outlined' style={{ borderColor: '#e0e0e0', backgroundColor: '#fff', padding: '0 20px' }}>
                  ...
                  {filterMultiSelectTabs?.length > 0 &&
                  filterColumnName === 'Academic Year' &&
                  operationName == 'multiSelect' ? (
                    <>
                      <Typography sx={{ paddingTop: '14px' }}>Value</Typography>
                      <AccordionDetails sx={{ margin: '0 4px', padding: 0 }}>
                        <div style={{ display: 'flex', justifyContent: 'flex-start', flexWrap: 'wrap' }}>
                          {filterMultiSelectTabs &&
                            filterMultiSelectTabs.map((status: any, index: number) => (
                              <StyledChipProps
                                ...
                                variant='filled'
                                color={
                                  selectedMultiSelectValues.includes(status.value.toString()) ? 'primary' : 'default'
                                }
                                sx={{ margin: '4px', cursor: 'pointer' }}
                                onClick={() => handleMultiSelectChipToggle(status.value)}
                              />
                            ))}
                        </div>
                      </AccordionDetails>
