...
import { getObjectByKeyVal, formatDate as formatDatee, getUserInfo, getObjectByKeyValNew } from 'src/utils/helper'
...
const Enquiries = ({ edit, view, selectedRowId, registration, app, authToken }: EnquiryDetails) => {
  ...
  const handleReopenEnquiryDialogOpen = async () => {
    const admittedStage = enquiryDetails?.enquiry_stages?.find(
      (stage: any) => stage.stage_name === 'Admitted or Provisional Approval'
    )

    if (admittedStage?.status != 'Open') {
      setApiResponseType({
        status: true,
        message: "Can't open the Lead once Admitted"
      })
      return
    }
    ...
    if (response?.status == 200) {
      ...
      setApiResponseType({
        status: true,
        message: response?.error?.error?.response?.message || 'Something Went Wrong!'
      })
    }
  ...
  const enquiry_status =
    enquiryCounts?.enquiry_status === 'Admitted' ? 'Provisional Admission' : enquiryCounts?.enquiry_status // this is a temporary fix for testing because of Sampada
  ...
  return (
    <Fragment>
      ...
      <Dialog open={open} fullScreen={fullScreen} fullWidth maxWidth='xl' sx={{ width: '100%' }}>
        <Box
          sx={{
            position: 'relative',
            maxHeight: '90vh',
