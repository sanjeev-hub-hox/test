...
import { Button, Fab, IconButton, Tooltip, Typography, Badge } from '@mui/material'
...
function EnquiryListing() {
  ...
  const [sortData, setSortData] = useState<{ field: string; sort: 'asc' | 'desc' } | null>(null)
  ...
  const [actionEnquiryType, setActionEnquiryType] = useState<any>(null)
  const [filteredColumns, setFilteredColumns] = useState<any>([])
  const [dataGridsCols, setDataGridCols] = useState<any>([])
  const [dataGridsColsMain, setDataGridColsMain] = useState<any>([])
  const [academicYearTabs, setAcademicYearTabs] = useState<any[]>([])
  ...
  const handleSchoolVisit = (params: GridRenderCellParams) => {
    setActionEnquiryId(params?.row?.id)
    setActionEnquiryType(params?.row?.enquiry_for)
  ...
  const fetchData = useCallback(
    async (
      ...
      filterData?: any,
      searchText?: string,
      sortInfo?: { field: string; sort: 'asc' | 'desc' } | null
    ) => {
      setGlobalState({ isLoading: true })
      try {
        const url = `marketing/enquiry/cc/list?page=${page}&size=${pageSize}${
          sortInfo?.field ? `&sort_by=${sortInfo.field}&order=${sortInfo.sort}` : ''
        }`
        ...
        const apiRequest = {
          ...
          data: {
            filters: filterData || [],
            // Always send search — backend handles empty/undefined gracefully
  ...
  useEffect(() => {
    ...
    if (year !== '') {
      fetchData(paginationModel?.page + 1, paginationModel.pageSize, modifyPayload, debouncedSearchTerm, sortData)
    }
  }, [fetchData, paginationModel, filterOptions, debouncedSearchTerm, year, sortData])
  ...
  }, [fetchData, paginationModel, filterOptions, debouncedSearchTerm, year, refresh, sortData])
  ...
  const columns: any = [
    ...
    },
    {
      ...
      minWidth: 150,
      field: 'overdueDays',
      headerName: 'Follow-up Overdue Days',
      headerClassName: getHeaderClassName('overdueDays'),
      cellClassName: (params: GridCellParams): string => {
        const headerName = params.colDef.headerName || ''
        const className = getCellClassName(headerName)
  ...
  ].map(col => ({ sortable: false, ...col }))

  const [data, setEnquiryListing] = useState<any[]>([])
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Guard: warn if too many records
      if (totalRows > 5000) {
        setApiResponseType({
          status: true,
          message: `${totalRows} records found. Download may take a while. Please wait...`
        })
      }

      const url = `marketing/enquiry/cc/download${
        sortData?.field ? `?sort_by=${sortData.field}&order=${sortData.sort}` : ''
      }`
      ...
      if (response?.status) {
        ...
        setApiResponseType({ status: true, message: `Downloaded ${response?.data?.content?.length} records successfully` })
      } else {
        setApiResponseType({ status: false, message: 'Download failed. Please try again.' })
      }
    } catch (error) {
      setApiResponseType({ status: false, message: 'Download failed. Please try again.' })
    } finally {
              ...
              <Grid item xs={12} sx={{ marginTop: '25px' }}>
                ...
                {dataGridsCols ? (
                  <DataGrid
                    ...
                    disableColumnFilter
                    sortingMode='server'
                    sortModel={sortData ? [{ field: sortData.field, sort: sortData.sort }] : []}
                    onSortModelChange={model => {
                      if (model && model.length > 0) {
                        setSortData({ field: model[0].field, sort: model[0].sort as 'asc' | 'desc' })
                      } else {
                        setSortData(null)
                      }
                      setPaginationModel(prev => ({ ...prev, page: 0 }))
