...
import { convertDateDD } from 'src/utils/helper'
...
export default function FilterComponent(props: FilterProps) {
  const { filterOpen, setFilterOpen, title, applyFilterOptions, academicYears, clearFilter, setFilteredColumns } = props
  // popover
  const handleClose = () => {
    setFilterOpen(null)
  }
  ...
  const [academicYearFilterOptions, setAcademicYearFilterOptions] = useState<any>([])
  const ss = ['2026 - 27', '2025 - 26', '2028 - 29']
  console.log('academicYearFilterOptions', academicYearFilterOptions)
  ...
  console.log('lklk2', selectedOptions)

  const handleToggle = (op: any) => {
    setSelectedOptions(prevSelected => {
      if (prevSelected.includes(op)) {
        return prevSelected.filter(item => item !== op)
      } else {
        return [...prevSelected, op]
      }
    })
  }
  ...
  const handleCoulmnFilter = (status: string) => {
    ...
    const index = options.findIndex(x => x.value === status)
    if (index >= 0) {
  ...
  useEffect(() => {
    if (academicYears) {
      const cleanedArray = academicYears.filter(item => item !== null)
      setAcademicYearFilterOptions(cleanedArray)
    }
  }, [academicYears])
                ...
                <Stack spacing={2}>
                  {filterCoulmnValue == 'academicYear' ? (
                    <>
                      <Stack spacing={2}>
                        <Grid item xs={12} sm={12}>
                          {academicYearFilterOptions && academicYearFilterOptions?.length ? (
                            <Autocomplete
                    ...
                    <>
                      ...
                      {filterCoulmnValue?.includes('Date') ? (
                ...
                <>
                  ...
                  >
                    <Typography color={'customColors.text3'}>
                      {options.find(op => op.value === status.column)?.name}
                    </Typography>
