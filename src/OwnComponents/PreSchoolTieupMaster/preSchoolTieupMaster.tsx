...
export default function CreatePreSchoolTieUpMaster() {
  ...
  const [, setLoadingCity] = useState(false)
  ...
  const handlePincodeChange = async (pincode: string) => {
    ...
    if (pincode.length === 6) {
      ...
      try {
        ...
        const cityName = response?.data[0]['attributes']?.District_Or_City?.data?.attributes?.name
        ...
        if (res?.data) {
          ...
          res.data.forEach((item: any) => {
            ...
            if (!seenCityIds.has(cityId)) {
              ...
              uniqueOptions.push({
                id: item.id?.toString(),
                name: item.attributes?.short_name || item.attributes?.name
              })
            }
          })
  ...
  const validateForm = async () => {
    ...
    // Preschool Name
    if (
      !formData.preschoolName ||
      formData.preschoolName.trim().length < 3 ||
      formData.preschoolName.trim().length > 100
    ) {
    ...
    if (
      !formData.preschoolDisplayName ||
      formData.preschoolDisplayName.trim().length < 3 ||
      formData.preschoolDisplayName.trim().length > 50
    ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await postRequest(params)

      setSuccessDialogOpen(true)
      // handleCancel();
    } catch (error: any) {
      alert(`Error creating Preschool Tie-Up: ${error?.response?.data?.error?.message || error.message}`)
    }
  }
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            <FormControl fullWidth required error={!!errors.applicableSchools}>
              ...
              {errors.applicableSchools && <Box sx={{ color: 'error.main', mt: 0.5 }}>{errors.applicableSchools}</Box>}
            </FormControl>
          </Grid>
        </Grid>
