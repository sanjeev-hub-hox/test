...
const ReferralListing = () => {
    ...
    () => [
      ...
      { field: 'sourceName', headerName: 'Source Name', width: 160 },
      {
        ...
        renderCell: (params: { row: ReferralRow }) => {
          ...
          const isManuallyVerified = params.row.manuallyVerifiedData?.manuallyVerified || params.row.manuallyVerified

          const isManuallyRejected = params.row.manuallyRejectedData?.manuallyRejected || params.row.manuallyRejected
                  ...
                  {isManuallyVerified && (
                    <>
                      ...
                      <Typography
                        variant='caption'
                        display='block'
                        sx={{ color: '#ffffff !important', opacity: 1, fontWeight: 500, mt: 1 }}
                      >
  ...
  const fetchReferrals = async () => {
    ...
    try {
      const params = {
        url: `marketing/enquiry/getAllReferrals?page=${paginationModel.page + 1}&pageSize=${
  ...
  const handleExportCSV = async () => {
    try {
      ...
      const headers = columns.filter((col: any) => col.field !== 'action').map((col: any) => col.headerName)
    ...
    >
      ...
      >
        ...
        >
          ...
          <IconButton onClick={handleCloseVerifyDialog} disabled={verifying} size='small'>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        ...
        <DialogContent sx={{ pt: 3, px: 3, pb: 2 }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 3, mb: 3 }}>
            <Box>
              ...
              <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.enrollment_number || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant='caption' sx={{ color: '#666', fontWeight: 600, display: 'block', mb: 0.5 }}>
                Enquiry No
              </Typography>
              <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.enquiry_number || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant='caption' sx={{ color: '#666', fontWeight: 600, display: 'block', mb: 0.5 }}>
                Student Name
              </Typography>
              <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.student_name || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant='caption' sx={{ color: '#666', fontWeight: 600, display: 'block', mb: 0.5 }}>
                Academic Year
              </Typography>
              <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.academic_year || '-'}</Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 3, mb: 3 }}>
            <Box>
              ...
              <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.school || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant='caption' sx={{ color: '#666', fontWeight: 600, display: 'block', mb: 0.5 }}>
                Board
              </Typography>
              <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.board || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant='caption' sx={{ color: '#666', fontWeight: 600, display: 'block', mb: 0.5 }}>
                Grade
              </Typography>
              <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.grade || '-'}</Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ borderTop: '1px solid #e0e0e0', pt: 2, mt: 2 }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
              <Box>
                ...
                <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.parent_name || '-'}</Typography>
              </Box>
              <Box>
                <Typography variant='caption' sx={{ color: '#666', fontWeight: 600, display: 'block', mb: 0.5 }}>
                  Phone Number
          ...
          <Box sx={{ borderTop: '1px solid #e0e0e0', pt: 2, mt: 2 }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
              <Box>
                ...
                <Typography sx={{ fontSize: '0.95rem' }}>{verifyDialog.row?.sourceName || '-'}</Typography>
              </Box>
              <Box>
                <Typography variant='caption' sx={{ color: '#666', fontWeight: 600, display: 'block', mb: 0.5 }}>
                  Phone Number
      ...
      >
        ...
        >
          ...
          <IconButton onClick={handleCloseRejectDialog} disabled={rejecting} size='small'>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
