...
type ApiClient = {
  get: (params: { url: string; serviceURL?: string; [key: string]: any }) => Promise<any>
  post: (params: { url: string; data?: any; serviceURL?: string; [key: string]: any }) => Promise<any>
}
...
export const RemoteApprovalWorkflow = ((props: ApprovalWorkflowProps) => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const mod: any = await import('amp-workflow-ui/dist/index.js')
      const C = mod.ApprovalWorkflow || mod.default?.ApprovalWorkflow || mod.default
      if (mounted) setComp(() => C)
    })()
