...
function DefaultFees({
  ...
  const [screenWidth] = useState<number>(typeof window !== 'undefined' ? window.innerWidth : 0)
  ...
  const fetchAllDefaultfees = async () => {
    ...
    const isIVT = enquiryDetails?.enquiry_type === 'IVT' || enquiryDetails?.enquiry_type?.slug === 'ivtEnquiry'
    const admissionTypeId = isIVT ? 8 : 1
    const admissionTypeCondition = isIVT
      ? `admission_type_id = ${admissionTypeId}`
      : `(admission_type_id = ${admissionTypeId} OR admission_type_id IS NULL)`

    const feeTypeIds = isIVT ? '(44,17,9)' : '(1,17,9,44)'
    const concessionResponse = await postRequest({
      ...
      data: {
        operator:
          operator +
          ` AND fee_type_id IN ${feeTypeIds} AND publish_start_date <= current_date AND publish_end_date >= current_date AND ${admissionTypeCondition}`
      }
    })
