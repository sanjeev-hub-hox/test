...
const TransportationDialog = ({
  ...
  const [errorTitle] = useState<any>('Something went wrong!')
  ...
  const [showStopCreationOption] = useState<boolean>(true)
  ...
  const [batchOptions] = useState<any[]>([])
  ...
  useEffect(() => {
    if (details && psaDetails) {
      ...
      const sType = getObjectByKeyVal(
        psaDetails?.feeCategory,
        'fee_category_id',
        details?.fee_category_id
      )?.fee_category
      if (sType == 'One way') {
        const stopPayload = {
