...
import { formatAmount, getCurrentAcademicYear, storeFile } from 'src/utils/helper'
...
function SampleDialog({ closeModal, header, enquiryDetails, academicYear, authToken }: customModal) {
  ...
  const [disocuntPopup] = useState<boolean>(false)
  const [enquiryID] = useState<any>(null)
  ...
  const concessionVerificationSubmit = async () => {
    ...
    const updatedFormDataWithoutDocumentTypeDetails = updatedFormData.map(({ ...rest }) => rest)
