...
const EnquirySchoolSource = ({ enquirySchoolSource, handleChange, val, formData, setFormData }: any) => {
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        const params = {
          url: `/api/ac-pre-school-tie-ups?pagination[limit]=-1&filters[validity_end_date][$gte]=${today}&filters[publishedAt][$notNull]=true`,
          serviceURL: 'mdm',
          headers: {
  ...
  return (
    <Autocomplete
      options={options}
      getOptionLabel={option => option.attributes.pre_school_name || ''}
      ...
      onChange={(event, newValue) => {
        ...
        setFormData((prev: any) => ({
          ...
          'enquiry_school_source.value': newValue.attributes.pre_school_name || '',
      ...
      renderOption={(props, option) => (
        <Box component='li' {...props} key={option.id} display='flex' justifyContent='space-between' width='100%'>
