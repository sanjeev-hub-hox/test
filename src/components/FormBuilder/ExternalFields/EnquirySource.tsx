...
import { searchObjectsByAttribute } from 'src/utils/helper'
...
const EnquirySource = ({ enquirySource, handleChange, val, setFormData, formData, infoDialog, enquiryType }: any) => {
  ...
  return options?.length ? (
    <FormControl fullWidth required error={Boolean(formData?.error?.['enquiry_source'])}>
      ...
      {formData?.error?.['enquiry_source'] ? (
        <FormHelperText error>{formData?.error?.['enquiry_source']}</FormHelperText>
      ) : null}
    </FormControl>
  ) : null
}
