...
const EnquirySourceType = ({ handleChange, val, formData, validation, enquiryType }: any) => {
  ...
  return enquirySources?.length ? (
    <FormControl fullWidth required error={Boolean(formData?.error?.['enquiry_source_type'])}>
      ...
      {formData?.error?.['enquiry_source_type'] ? (
        <FormHelperText error>{formData?.error?.['enquiry_source_type']}</FormHelperText>
      ) : null}
    </FormControl>
  ) : null
}
