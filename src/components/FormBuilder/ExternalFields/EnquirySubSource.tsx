...
const EnquirySubSource = ({ enquirySubSource, handleChange, val, formData, setFormData, enquiryType }: any) => {
      ...
      if (enquirySubSource) {
        ...
        if (response) {
          ...
          if (response?.data?.length == 0) {
            ...
            handleChange(
              'enquiry_sub_source',
              singleOption?.attributes?.enquiry_sub_source?.data?.id,
              [],
              false,
              null,
              {
  ...
  const isFieldMatchingSubSource = (field: string, subSource: any) => {
    const s = String(subSource || '')
    if (s.includes('Employee')) return field === 'enquiry_employee_source'
    if (s.includes('Parent')) return field === 'enquiry_parent_source'
    if (s.includes('Corporate')) return field === 'enquiry_corporate_source'
    if (s.includes('Pre School')) return field === 'enquiry_school_source'
    return false
  }

  return options?.length ? (
    <FormControl fullWidth required error={Boolean(formData?.error?.['enquiry_sub_source'])}>
      ...
      {formData?.error?.['enquiry_sub_source'] ? (
        <FormHelperText error>{formData?.error?.['enquiry_sub_source']}</FormHelperText>
      ) : null}
    </FormControl>
  ) : null
}
