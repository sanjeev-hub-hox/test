      ...
      <Grid item xs={12} md={4}>
        <FormControl component='fieldset'>
          <FormLabel component='legend' sx={{ mb: 1 }}>
            Is The Student Staff Child?
          </FormLabel>
          <RadioGroup
            row
            name='is_staff_child'
            value={formData['is_staff_child'] || 'no'}
            onChange={e => {
              handleChange('is_staff_child', e.target.value, validationArray, false)
            }}
          >
      ...
      {formData['is_staff_child'] === 'yes' && (
        <Grid item xs={12} md={4}>
          <TextField
            ...
            required
            onChange={e => {
              handleChange('employee_id', e.target.value, validationArray, false)
            }}
            error={Boolean(formData?.error?.['employee_id'])}
            helperText={formData?.error?.['employee_id']}
            InputProps={{
              endAdornment: formData['employee_details'] ? (
                <CheckCircleIcon sx={{ color: 'green' }} />
              ) : formData['error']?.['employee_id'] === 'Invalid Employee ID' ? (
