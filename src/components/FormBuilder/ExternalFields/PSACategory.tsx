...
const PSACategory = ({ psaSubType, handleChange, val, setFormData, formData, infoDialog }: any) => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      const params = {
        url: `/api/ac-spa-categories?fields[1]=name&fields[2]=short_name`,
        serviceURL: 'mdm',
        headers: {
  ...
  return options?.length ? (
    <FormControl fullWidth required>
      <InputLabel id='dependent-option-label'>PSA Category</InputLabel>
      <Select
        name={'psa_category'}
        ...
        error={Boolean(formData?.error?.['psa_category'])}
        onChange={e => {
          setFormData({ ...formData, enquiry_sub_source: '' })

          handleChange('psa_category', e.target.value, [], false, null, {
