      ...
      try {
        ...
        if (res && res?.data?.length) {
          setFormData((prevState: any) => ({
            ...
            [`${fieldName.replace('pin_code', 'state')}.value`]: res.data[0].attributes.State?.data?.attributes?.name,
            [`${fieldName.replace('pin_code', 'city')}.id`]: res.data[0].attributes.District_Or_City?.data?.id,
        ...
        <Grid item xs={12}>
          <FormControl sx={{ width: '33%' }} variant='outlined'>
            ...
            <Select
              ...
              onChange={e => handleParentFieldChange('residential_details.is_permanent_address', e.target.value, [])}
